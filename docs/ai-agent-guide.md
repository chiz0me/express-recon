...
- every `applications[].id` and which app IDs are in scope;
...
Prefer CLI `scan-org --out <dir>` so detailed reports are persisted and released
one repository at a time. Begin with `organization-inventory.json` and verify:
...
- the distinction between `not-express` and `inconclusive`;
- each Express repository's artifact paths before reading its detailed routes.
...
In this context the CLI requires `--out` and defaults progress to `none`, so the
agent does not need to remember separate safety flags. Without `--out`, full
per-repository scans would remain embedded in stdout and memory, so agent
context fails before enumeration instead. An explicit `--progress` or
`--no-progress` always takes precedence. If the user needs live progress, write
JSONL to a file instead of returning the entire stream to the model:

```bash
EXPRESS_RECON_CONTEXT=agent express-recon scan-org \
  --org <org> --out <outDir> --progress json 2>scan-progress.jsonl
```
...
`summary` from `organization-inventory.json`, then select repository entries
whose status is `express`, `failed`, or `inconclusive`. Open detailed
...
non-Express entry to a model. Run `review-middleware` and model-assisted
...
hybrid mode unless a human explicitly opts into executing a trusted local app.
The runtime worker is process containment, not an OS sandbox.
...
> Inventory every Express application. Check coverage first, preserve app
> identity, and list partial paths separately. Do not make auth claims.
